 ...
 */
        ...
        public void read(byte data)
        {
            switch (state)
            {
                ...
                case 5:
                    msg.length = data;
                    crcpacket = crc.Accumulate(data, crcpacket);
                    msg.payloads = new u8[msg.length];
                    length = 0;
                    state++;
                    break;
                case 6:
        ...
        public void ProcessMessage(piksimsg msg)
        {
            ...
            {
                var test = msg.payloads.ByteArrayToStructures<sbp_gps_time_t>(0);
            ...
            {
                var test = msg.payloads.ByteArrayToStructures<sbp_pos_llh_t>(0);
            ...
            {
                var test = msg.payloads.ByteArrayToStructures<sbp_pos_ecef_t>(0);

                //Console.WriteLine(test.x + " " + test.y + " " + test.z);
            }
            else if ((MSG)msg.msg_type == MSG.SBP_VEL_NED)
            {
                var test = msg.payloads.ByteArrayToStructures<sbp_vel_ned_t>(0);

                //Console.WriteLine(test.n + " " + test.e + " " + test.d);
            }
            else if ((MSG)msg.msg_type == MSG.SBP_VEL_ECEF)
            {
                var test = msg.payloads.ByteArrayToStructures<sbp_vel_ecef_t>(0);

                //Console.WriteLine(test.x + " " + test.y + " " + test.z);
            }
            else if ((MSG)msg.msg_type == MSG.SBP_BASELINE_NED)
            {
                var test = msg.payloads.ByteArrayToStructures<sbp_baseline_ned_t>(0);

                //Console.WriteLine(test.n + " " + test.e + " " + test.d);
            }
            else if ((MSG)msg.msg_type == MSG.SBP_BASELINE_ECEF)
            {
                var test = msg.payloads.ByteArrayToStructures<sbp_baseline_ecef_t>(0);
                if (consoleoutput)
                {
            ...
            else if ((MSG)msg.msg_type == MSG.SBP_MSG_OBS)
            {
                ...
                var hdr = msg.payloads.ByteArrayToStructures<observation_header_t>(0);
                ...
                //todo add tow check
                if (count > 0 && msgobs.payloads != null)
                {
                    // resize msgobs payload to include current msg obs
                    int currentpayloadend = msgobs.payloads.Length;
                    Array.Resize<byte>(ref msgobs.payloads, msgobs.payloads.Length + obscount * lenobs);
                    Array.Copy(msg.payloads, lenhdr, msgobs.payloads, currentpayloadend, obscount * lenobs);
                    msgobs.length += (byte)(obscount * lenobs);
                }

                for (int a = 0; a < obscount; a++)
                {
                    var ob = msg.payloads.ByteArrayToStructures<packed_obs_content_t>(lenhdr + a * lenobs);
                ...
                if (count == (total - 1) && msgobs.payloads != null)
                {
            ...
            {
                var bpos = msg.payloads.ByteArrayToStructures<msg_base_pos_t>(0);
            ...
            {
                var test = msg.payloads.ByteArrayToStructures<msg_iar_state_t>(0);
            ...
            else if ((MSG)msg.msg_type == MSG.MSG_PRINT)
            {
                if (consoleoutput)
                {
                    ...
                    Console.Write(printline + " " + ASCIIEncoding.ASCII.GetString(msg.payloads));
                }
            ...
            else if ((MSG)msg.msg_type == MSG.MSG_TRACKING_STATE)
            {
                ...
                {
                    var test = msg.payloads.ByteArrayToStructures<tracking_state_msg_t>(a);
            ...
            {
                var test = msg.payloads.ByteArrayToStructures<msg_uart_state_t>(0);
            ...
            {
                var test = msg.payloads.ByteArrayToStructures<msg_thread_state_t>(0);
            ...
            {
                var test = msg.payloads.ByteArrayToStructures<acq_result_msg_t>(0);
                if (consoleoutput)
                {
            ...
            {
                string test = ASCIIEncoding.ASCII.GetString(msg.payloads);
            ...
            {
                // Console.Clear();
                if (msg.payloads.Length == 8)
                {
                    var value = BitConverter.ToDouble(msg.payloads, 0);
                    string debug = ASCIIEncoding.ASCII.GetString(msg.payloads, 8, msg.payloads.Length - 8);
                    if (consoleoutput)
                    {
            ...
            else if (msg.msg_type == 0x207)
            {
                ...
                var meas = msg.payloads.ByteArrayToStructures<channel_measurement_t>(0);
                if (consoleoutput)
                {
            ...
            else if (msg.msg_type == 0x208)
            {
                ...
                var test = msg.payloads.ByteArrayToStructures<navigation_measurement_t>(0);
            ...
            else if ((MSG)msg.msg_type == MSG.SBP_MSG_EPHEMERIS)
            {
                ...
                var test = msg.payloads.ByteArrayToStructures<ephemeris_t>(0);

                eph[test.sid + 1] = test;

                File.WriteAllBytes((test.sid + 1) + ".eph", msg.payloads);
