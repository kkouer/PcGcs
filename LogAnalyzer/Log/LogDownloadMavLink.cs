                ...
                this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate()
{
    try
    {
        ...
        {
            GCSMainForm.comPort.Progress += comPort_Progress;

            status = serialstatus.Reading;

            // get df log from mav
            var ms = GCSMainForm.comPort.GetLog(no);
            ...
            // set log fn
            byte[] hbpacket = GCSMainForm.comPort.getHeartBeat();

            MAVLink.mavlink_heartbeat_t hb = (MAVLink.mavlink_heartbeat_t)GCSMainForm.comPort.DebugPacket(hbpacket);

            logfile = MainV2.LogDir + Path.DirectorySeparatorChar
             + GCSMainForm.comPort.MAV.aptype.ToString() + Path.DirectorySeparatorChar
        ...
        {
            if (CustomMessageBox.Show("是否清空?", "确定", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
            {
                try
                {
                    GCSMainForm.comPort.EraseLog();
