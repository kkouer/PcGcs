...
namespace MissionPlanner
{
    public partial class GridUI : Form
    {
        ...
        void savesettings()
        {
            ...
            MainV2.config["grid_alt"] = NUM_altitude.Value.ToString();
            MainV2.config["grid_angle"] = NUM_angle.Value.ToString();
            ...
            MainV2.config["grid_overshoot1"] = NUM_overshoot.Value.ToString();
            MainV2.config["grid_overshoot2"] = NUM_overshoot2.Value.ToString();
            MainV2.config["grid_overlap"] = num_overlap.Value.ToString();
            MainV2.config["grid_sidelap"] = num_sidelap.Value.ToString();
            MainV2.config["grid_spacing"] = NUM_spacing.Value.ToString();

            MainV2.config["grid_startfrom"] = CMB_startfrom.Text;
            ...
            // Plane Settings
            MainV2.config["grid_min_lane_separation"] = NUM_Lane_Dist.Value.ToString();
        }
        ...
        private void AddWP(double Lng, double Lat, double Alt)
        {
            ...
            else
            {
                if (CHK_copter_headinghold.Checked)
                {
                    this.gcs.AddCommand(MAVLink.MAV_CMD.CONDITION_YAW, Convert.ToInt32(TXT_headinghold.Text), 0, 0, 0, 0, 0, 0);
                    WPNo++;
                }

                if (NUM_copter_delay.Value > 0)
                {
                    this.gcs.AddCommand(MAVLink.MAV_CMD.WAYPOINT, (double)NUM_copter_delay.Value, 0, 0, 0, Lng, Lat, Alt * CurrentState.multiplierdist);
                    WPNo++;
                }
                else
                {
                    this.gcs.AddCommand(MAVLink.MAV_CMD.WAYPOINT, 0, 0, 0, 0, Lng, Lat, (int)(Alt * CurrentState.multiplierdist));
                    WPNo++;
                }
            }
        }
        ...
        string secondsToNice(double seconds)
        {
            ...
            if (hours > 0)
            {
                return hours + ":" + mins.ToString("00") + ":" + secs.ToString("00") + " 时";
            }
            else if (mins > 0)
            {
                return mins + ":" + secs.ToString("00") + " 分";
            }
            else
            {
        ...
        private void BUT_save_Click(object sender, EventArgs e)
        {
            ...
            if (MissionPlanner.Controls.InputBox.Show("相机名称", "输入相机名称", ref camname) != System.Windows.Forms.DialogResult.OK)
                return;
