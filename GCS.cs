...
namespace MissionPlanner
{
    public partial class GCS : Form
    {
        ...
        {
            if (Commands.RowCount <= 3)
            {
                CustomMessageBox.Show("未发现航点!");
                return;
            }
            ...
            float AntiWind = comPort.MAV.cs.wind_dir + 180 > 360 ? comPort.MAV.cs.wind_dir + 180 - 360 : comPort.MAV.cs.wind_dir + 180;
            PointLatLng ReturnPoint = calDistanceToLogLat(0.24f, LandingPoint, AntiWind);
            CustomMessageBox.Show(" 风向 : " + comPort.MAV.cs.wind_dir.ToString() + " 度\n 风力 : " + comPort.MAV.cs.wind_vel.ToString() + " 米/秒");
        ...
        /// <param name="startPoint">起点经纬度</param>
        /// <param name="angle">角度,单位度数</param>
        /// <returns></returns>
        private PointLatLng calDistanceToLogLat(float distance, PointLatLng startPoint, double angle)
        {
